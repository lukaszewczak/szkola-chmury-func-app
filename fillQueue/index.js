...
module.exports = async function(context, timer) {
  ...
  const messages = response.body
    ...
    .map(event => {
      const title =
        (event.payload.pull_request && event.payload.pull_request.title) ||
        (event.payload.issue && event.payload.issue.title);

      const type =
        event.type === 'IssuesEvent' ? 'Nowe issue' : 'Nowy pull request';

      return `${type}: Uzytkownik '${event.actor.login}', repo '${
        event.repo.name
      }', title '${title}' `;
    });
